...
export class Feature1B04SpecialKeys {
    ...
    async getCtrlIdInfo(ctrlIdIndex) {
        const response = await this.hidppDevice.sendCommand(this.featureIndex, FUNCTION_GET_INFO, [ctrlIdIndex & 0xff]);
    ...
    async getCtrlIdReporting(ctrlId) {
        const response = await this.hidppDevice.sendCommand(this.featureIndex, FUNCTION_GET_REPORTING, [
            (ctrlId >> 8) & 0xff,
            ctrlId & 0xff
        ]);
    ...
    async setCtrlIdReporting(ctrlId, reporting, remap = 0, reporting2 = 0) {
        await this.hidppDevice.sendCommand(this.featureIndex, FUNCTION_SET_REPORTING, [
            (ctrlId >> 8) & 0xff,
            ctrlId & 0xff,
            reporting & 0xff,
            (remap >> 8) & 0xff,
    ...
    async configureDiversionReporting() {
        ...
        for (let i = 0; i < controlCount; i += 1) {
            ...
            try {
                ...
                // Keep setup resilient for partially supported controls.
            }
        }
    ...
    async restoreReporting(snapshots) {
        ...
        for (const snapshot of snapshots) {
            try {
                await this.setCtrlIdReporting(snapshot.ctrlId, snapshot.reporting, snapshot.remap, snapshot.reporting2);
            } catch (_error) {
    ...
    handleDivertedButtonsEvent(params) {
        ...
        for (const handler of this.keyChangeHandlers) {
            handler(controlIds);
        }
    }
}
